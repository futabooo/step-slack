...
export GIT_COMMIT_HASH=$(echo "$WERCKER_GIT_COMMIT" | cut -c1-7)
export GIT_BRANCH_URL="https://$WERCKER_GIT_DOMAIN/$WERCKER_GIT_OWNER/$WERCKER_GIT_REPOSITORY/commit/$WERCKER_GIT_COMMIT|$GIT_COMMIT_HASH"
export MESSAGE="<$ACTION_URL|$ACTION> for $WERCKER_APPLICATION_NAME by $WERCKER_STARTED_BY has $WERCKER_RESULT on branch $WERCKER_GIT_BRANCH <$GIT_BRANCH_URL>"
